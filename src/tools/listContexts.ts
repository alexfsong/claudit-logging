...
export async function listContexts(vaultPath: string) {
  ...
  if (technical.length > 0) {
    ...
    technical.forEach((c, i) => {
      lines.push(
        `  [${i + 1}] ${c.title.padEnd(24)} (${c.area})  ${String(c.session_count).padStart(3)} sessions  last: ${c.last_used}`
      );
      lines.push(`       id: ${c.id}`);
    });
    lines.push("");
  ...
  if (topics.length > 0) {
    ...
    topics.forEach((c, i) => {
      lines.push(
        `  [${technical.length + i + 1}] ${c.title.padEnd(24)} (${c.area})  ${String(c.session_count).padStart(3)} sessions  last: ${c.last_used}`
      );
      lines.push(`       id: ${c.id}`);
    });
  }
...
export async function createContext(vaultPath: string, args: any) {
...
export async function updateContext(vaultPath: string, args: any) {
