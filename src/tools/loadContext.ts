...
export async function loadContext(vaultPath: string, args: any) {
  ...
  try {
    ...
    for (const r of results) {
      const p = r.metadata.path as string;
      if (seen.has(p) || p.includes("/contexts/")) continue;
      seen.add(p);

      const score = (1 - r.distance).toFixed(2);
      const type = (r.metadata.type as string)?.toUpperCase() ?? "NOTE";
...
export function resolveContextPath(vaultPath: string, contextId: string): string {
  if (contextId.endsWith(".md")) return path.join(vaultPath, "contexts", contextId);
