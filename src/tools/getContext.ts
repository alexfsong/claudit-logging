...
import { getDb, type KnowledgeRow, type SessionRow } from "../db.js";
...
export function getContext(args: {
  ...
  if (knowledge.length > 0) {
    ...
    lines.push("");
    lines.push(
      `Use \`search_knowledge\` to find specific items, or reference by [id].`
    );
  } else {
    lines.push(
      "_No knowledge stored yet. Use `add_knowledge` to build the knowledge base._"
    );
  }

  lines.push("");
  lines.push(
    `**${annotatedCount}** files annotated. Use \`get_project_map\` to see the full structure.`
  );
