...
export async function extractSolution(vaultPath: string, args: any) {
  const now = new Date();
...
async function linkSolutionToSession(vaultPath: string, sessionPath: string, solutionPath: string) {
  ...
  if (!Array.isArray(fm.solutions_extracted)) fm.solutions_extracted = [];
