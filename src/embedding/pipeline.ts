...
async function ensureCollection() {
  ...
  if (!getRes.ok) throw new Error(`ChromaDB get collection failed: ${getRes.status}`);
  const col: any = await getRes.json();
...
export async function chromaUpsert(
  ...
  if (!res.ok) throw new Error(`ChromaDB upsert failed: ${res.status} ${await res.text()}`);
}
...
export async function chromaQuery(
  ...
  const body: any = { query_embeddings: [embedding], n_results: k, include: ["documents", "metadatas", "distances"] };
...
export async function chromaDeleteByPath(notePath: string) {
  const colId = await ensureCollection();
  ...
  if (!res.ok) throw new Error(`ChromaDB delete failed: ${res.status} ${await res.text()}`);
}
...
interface Chunk {
  index: number;
  text: string;
  type: "frontmatter" | "body" | "solution" | "context";
}

function chunkNote(frontmatter: Record<string, any>, body: string, noteType: string): Chunk[] {
  const chunks: Chunk[] = [];
...
export async function embedNote(vaultPath: string, notePath: string) {
  ...
  let noteType = "body";
  if (notePath.includes("/solutions/")) noteType = "solution";
  else if (notePath.includes("/contexts/")) noteType = "context";
  else if (notePath.includes("/sessions/")) noteType = "session";
  ...
  await chromaUpsert(
    chunks.map((c) => ({
      ...
      metadata: {
        path: relPath,
        area: (frontmatter.area as string) ?? "unknown",
        type: noteType,
        chunk_type: c.type,
...
export async function embedAllNotes(vaultPath: string): Promise<number> {
