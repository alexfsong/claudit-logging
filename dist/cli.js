...
async function main() {
    ...
    switch (command) {
        ...
        case "list": {
           ...
           LIMIT 100`)
                .all(...[project, ...(typeFilter ? [typeFilter] : [])]);
            if (!rows.length) {
        ...
        case "delete": {
            ...
            if (isNaN(id)) {
                console.error("Usage: claudit delete <id>");
                process.exit(1);
            }
            const row = db.prepare("SELECT title FROM knowledge WHERE id = ?").get(id);
            if (!row) {
        ...
        case "sessions": {
            const project = detectProject();
            const rows = db
                .prepare(`SELECT * FROM sessions WHERE project = ? ORDER BY created_at DESC LIMIT 20`)
                .all(project);
            if (!rows.length) {
        ...
        case "info": {
            ...
            const { knowledgeCount } = db.prepare("SELECT COUNT(*) AS knowledgeCount FROM knowledge").get();
            const { sessionCount } = db.prepare("SELECT COUNT(*) AS sessionCount FROM sessions").get();
            const { fileCount } = db.prepare("SELECT COUNT(*) AS fileCount FROM file_roles").get();
            const { projectCount } = db.prepare("SELECT COUNT(DISTINCT project) AS projectCount FROM knowledge").get();
