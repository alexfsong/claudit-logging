...
export async function updateSession(vaultPath, args) {
    ...
    if (args.rating !== undefined)
        fm.rating = args.rating;
    if (args.gaps_noticed !== undefined)
        fm.gaps_noticed = args.gaps_noticed;
    let updatedBody = body;
    if (args.what_worked)
        updatedBody = replaceSection(updatedBody, "What worked", args.what_worked);
    if (args.what_didnt)
        updatedBody = replaceSection(updatedBody, "What didn't", args.what_didnt);
    if (args.follow_up)
        updatedBody = replaceSection(updatedBody, "Follow-up needed", args.follow_up);
    // Rebuild the file
    ...
    const suggestion = args.rating >= 4
