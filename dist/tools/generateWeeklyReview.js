...
export async function generateWeeklyReview(vaultPath, args) {
...
${(review.top_projects ?? []).map((p) => `- ${p}`).join("\n")}

## Recurring workflows spotted
${(review.recurring_workflows ?? []).length > 0
        ? review.recurring_workflows.map((w) => `- ${w}`).join("\n")
...
${(review.suggested_automations ?? []).map((a) => `- ${a}`).join("\n")}

## Questions worth exploring
${(review.questions_worth_exploring ?? []).map((q) => `- ${q}`).join("\n")}
...
async function collectSessionsInRange(vaultPath, startDate, endDate) {
...
function computeStats(sessions) {
    ...
    for (const s of sessions) {
        ...
        if (s.context_provided) {
            for (const key of Object.keys(contextGaps)) {
                if (!s.context_provided[key])
                    contextGaps[key]++;
            }
        }
    }
